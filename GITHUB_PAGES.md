...
In the repo's [_Settings > Pages_ section](https://github.com/The-AI-Alliance/the-ai-alliance.github.io/settings/pages), use the menu to select the branch from which you want to publish the website. By default, we use `main` for the publication branch, while some other microsites use a different branch, i.e., `latest`. Finally, select the `docs` folder in the dropdown menu to the right, which is the root folder for the pages.
